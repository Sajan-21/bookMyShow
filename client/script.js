...
//add data
async function addMovie(base64string) {
        ...
        for(let i = 0; i < moviesData.length; i++) {
            ...
            rows = rows + `
                <div onclick="singleMoviePage('${moviesData[i]._id}')" class="text-center box-width">
                    ...
                    <div class="">
                        <div class="bg-dark text-light rounded-bottom rating text-center">${moviesData[i].rating}/10</div>
                    </div>
                    <div>
                        <div>${moviesData[i].title}</div>
                        <div>${moviesData[i].category.category}</div>
                    </div>
                </div>
            `;
...
async function categorized(event) {
    ...
    try {
        ...
        let response = await fetch(`/categorized/?category=${category}&language=${language}`);
        console.log("response : ",response);
